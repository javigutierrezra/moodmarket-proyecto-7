...
function Home() {
  ...
  const { addToCart } = useContext(CartContext) // ✅ conectar carrito
  ...
  useEffect(() => {
    axios
      ...
      .then(res => {
        ...
        console.log("Productos cargados desde backend:", res.data) // 👈 para revisar que llegan
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <h2 className="text-3xl font-bold mb-6 text-gray-800">Catálogo MoodMarket</h2>
      ...
      <div className="mb-6 flex gap-4 flex-wrap">
        ...
        <button
          ...
          className={`px-4 py-2 rounded ${
            selectedMood === "feliz" ? "bg-yellow-400 text-white" : "bg-yellow-200"
          }`}
        >
          Feliz
        </button>
        <button
          ...
          className={`px-4 py-2 rounded ${
            selectedMood === "relajado" ? "bg-green-400 text-white" : "bg-green-200"
          }`}
        >
          Relajado
        </button>
        <button
          ...
          className={`px-4 py-2 rounded ${
            selectedMood === "energetico" ? "bg-red-400 text-white" : "bg-red-200"
          }`}
        >
          Energético
        </button>
        <button
          ...
          className={`px-4 py-2 rounded ${
            selectedMood === "triste" ? "bg-blue-400 text-white" : "bg-blue-200"
          }`}
        >
          Triste
        </button>
        ...
        {filteredProducts.length > 0 ? (
          filteredProducts.map(product => (
            ...
            >
              ...
              {/* Botón agregar al carrito con console.log */}
              <button
                onClick={() => {
                  console.log("Agregando al carrito:", product) // 👈 revisa en consola
